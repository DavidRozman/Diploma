...
podan kot funkcija na nekem intervalu. Ker je reševanje takih enačb precej bolj zahtevno kot reševanje navadnih 
diferencialnih enačb, je tu zelo pogosta uporaba numeričnih metod. Pogoji za obstoj in enoličnost rešitve so podobni kot
pri navadnih diferencialnih enačbah, vendar ta pogosto ne bo odvedljiva ali zvezna. Do rešitev lahko pridemo tudi s 
...
pa skoraj vedno zahteva uporabo numeričnih metod. Diferencialne enačbe z zamikom so tudi koristne pri analiziranju 
...
result may not be expressed as an elementary function. Certain equations can still be solved using specific 
...
\begin{definicija}
    ...
    delay dif\mbox{}ferential equation.), kjer je $\tau$ zamik.
...
\begin{zgled}
    ...
    \end{figure}
\end{zgled}
Opazimo, da začetni pogoj tu veliko bolj vpliva na obliko končne rešitve, saj je podan kot funkcija in 
...
\begin{trditev}
    ...
    \end{itemize}
\end{trditev}
Dokaz trditve se nahaja v viru \cite{envir}

\subsection{Obstoj in enoličnost rešitve}
Ker so linearne DDE oblike
\[\dot{x}(t)=f(t,x(t),x(t-\tau)), \quad t\in[\xi,\xi+a],\]
za $a,\tau>0$ in $\xi>=0$ in z začetnim pogojem
\[x(t)=\phi(\tau), \quad t\in[\xi-\tau,\xi],\]
na njih ne moremo takoj aplicirati eksistenčnega izreka iz četrtega poglavja. Zato jih najprej preoblikujemo.
Za $t\in[\xi,\xi+\tau]$ velja 
\begin{equation*}
    \begin{split}
...
diferencialne enačbe. Torej je rešitev na $[\xi,\xi+\tau]$ enolična. Induktivno lahko kot pri dokazu eksistenčnega 
izreka v četrtem poglavju rešitev razširimo na $[\xi,\infty)$.
...
prej rast upočasni skoraj na 0, ko populacija doseže nosilnost okolja ($K$) za dano vrsto. Zato konstantno $a$ v 
...
\end{equation}
kjer je $K=a/b$. Enačbi \eqref{eq8} pravimo logistična enačba.
...
\begin{figure}[h]
    \includegraphics[width=12cm, height=10cm]{logdelay.JPG}
