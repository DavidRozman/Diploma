...
\noindent Pomagamo si z linearizacijo. To pomeni, da rešitev zapišemo v obliki $$x(t)=K+p(t),$$ pri čemer predpostavimo, da je 
...
\begin{itemize}
    \item $p(t)$ oscilira za $-a\tau<1/e$.
    \item $p(t)$ ne oscilira za $-a\tau\geq1/e$.
\end{itemize} 
Tovrstno dogajanje potrjuje tudi spodnja skica, ki je bila izdelana v okolju Matlab.

\begin{figure}[h]
    \includegraphics[width=\textwidth]{slikca1.eps}
\end{figure}

\noindent Z zeleno barvo je označena prava rešitev logistične enačbe z zamikom, z modro pa 
približek za funkcijo $p(t)$. Opazimo, da se rešitve 
tudi bližajo vrednosti $K$, vendar hkrati okoli nje nihajo. Funkcija $p(t)$ se obnaša podobno kot 
rešitev v vseh primerih, razen zadnjem, kjer je $a\tau=-2.25<-\pi/2$, zaradi česar je po trditvi 4.5 $x=K$
nestabilna rešitev. 
