...
\begin{zgled}
    ...
    \begin{equation} \label{eq3}
     ...
     \int_{1}^{\infty}\dot{x}(t)e^{-st}dt = \int_{1}^{\infty}x(t-1)e^{-st}dt.
        \end{split}
    \end{equation}
    Z uvedbo nove spremenljivke $r=t-1$ preoblikujemo desno stran enačbe:
    \begin{equation*}
        \begin{split}
            \int_{1}^{\infty}x(t-1)e^{-st}dt &= \int_{0}^{\infty}x(r)e^{-s(r+1)}dr \\
    ...
    Vidimo, da je rešitev DDE (1) izražena s svojimi vrednostmi na intervalu [0,1], torej njenega
    začetnega pogoja. Za razliko od analognega zgleda iz 3. poglavja, pa nam tu ni uspelo rešitve
