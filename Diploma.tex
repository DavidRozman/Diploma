...
\begin{proof}[Ideja dokaza]
    Linearni operator $L$ $n$-krat odvajamo po $\lambda$, nato vanj vstavimo $\mu^{n}\lambda^{\lambda \mu}$.
\end{proof}

\noindent $h$ je analitična funkcija kompleksne spremenljivke $\lambda$, zato zanjo veljajo naslednje lastnosti:
\begin{enumerate}
    \item[(A)] Za $\forall R > 0$, je množica ničel, ki zadoščajo pogoju $|\lambda|\leq R$ končna. Torej je
            množica ničel funkcije $h$ števna (pogosto tudi končna) množica.
    \item[(B)] Če je množica ničel neskončna (označimo jo z $\{\lambda_{n}\}_{n=1}^{\infty}$), potem
    $|\lambda_{n}| \to \infty$. Ker je $|\beta|e^{-\text{Re}(\lambda_{n})}=|\lambda_n|$, sledi, da
    $\text{Re}(\lambda_n) \to -\infty$. Prav tako $\forall a \in \R$ obstaja končno mnogo ničel $\lambda_n$,
...
\begin{trditev}
    ...
    \begin{enumerate}
        ...
        \item Če je $\beta = 1/e$, potem obstaja ena realna ničla reda 2 in sicer $\lambda = -1$.
...
\begin{proof}[Dokaz]
    \begin{enumerate}
        \item Funkcija $h$ je naraščajoča in velja $h \to \pm \infty$, ko $x \to \pm \infty$. Obstaja 
        torej natanko ena realna ničla. Ker je $\beta < 0$, je $\lambda = -\beta e^{-\lambda} >0$.
        \item Funkcija $h$ doseže minimum pod abscisno osjo, zatp obstajata natanko dve realni ničli. Ker je 
        $\beta > 0$, je $\lambda = -\beta e^{-\lambda} <0$. Drugi del trditve sledi iz naslednjega:
        $ \beta = -\lambda e^{\lambda} \to 0 \Leftrightarrow \lambda \to 0$ ali $\lambda \to -\infty$.
        \item Funkcija $h$ doseže minimum na abscisni osi, torej obstaja natanko ena realna ničla. Ko v
        $h$ vstavimo $\beta = 1/e$, je rešitev $\lambda = -1$. Velja tudi $h'(-1)=0$ in $h''(-1)=1$, torej
        je ničla reda 2.
        \item Funkcija $h$ doseže minimum nad abscisno osjo, torej realne ničle ne obstajajo.
    \end{enumerate}
\end{proof}
...
\begin{trditev}
    ...
    \begin{enumerate}
        \item Če je $0<\beta < \pi /2$, potem $\exists \delta > 0$, da je Re($\lambda$) $\leq -\delta$
        za vse ničle $\lambda$.
        \item Če je $\beta = \pi /2$, potem sta $\lambda_{1,2}=\pm i\pi /2$ ničli reda 1.
        \item Če je $\beta > \pi /2$, potem so ničle oblike $\lambda = a + ib$, kjer je $x>0$ in 
         $y \in (\pi /2, \pi)$.
    \end{enumerate} 
\end{trditev}
...
je $\cos(b) \leq 0 < \sin(b)$. Torej $b \in S = \bigcup_{n=0}^{\infty}\{[\pi / 2,\pi)+2n\pi\}.$ Velja
tudi $\frac{\sin(b)}{b}=\frac{e^{x}}{\beta}$. Ker je tudi $\frac{d}{db}\frac{\sin(b)}{b} < 0$ za $b \in S$
in $\frac{\sin(\pi /2)}{\pi / 2} = \frac{2}{\pi}$, je torej $\frac{\sin(b)}{b} \leq \frac{2}{\pi} \quad
\forall b \in S$.
Dobili smo naslednjo neenakost: 
\[\frac{1}{\beta} \leq \frac{e^x}{\beta} = \frac{\sin(b)}{b} \leq \frac{2}{\pi}.\]
...
\begin{equation*}
    \begin{split}
        ...
        \beta(\theta) &= \frac{\pi - \theta}{\sin(\theta)}e^{x(\theta)}. 
    \end{split}      
\end{equation*}
...
\begin{definicija}
    Dana je DDE $\dot{x}(t)=f(t,x(t-\tau))$ in njena rešitev $\phi(t)$. $\phi$ je stabilna rešitev, če 
    $\forall \varepsilon > 0 \quad \exists \delta > 0$ tako da, če je $\psi$ neka druga rešitev in je 
    $|\psi(t_0)-\phi(t_0)| < \delta$, je potem $|\psi(t)-\phi(t)| < \varepsilon$ za $t \geq t_0$. \\
    Če velja
    še $\lim_{x \to \infty}|\psi(t)-\phi(t)|=0$, je rešitev asimptotsko stabilna.
\end{definicija}
...
\begin{proof}
    ...
    Za realne $\lambda$ torej rešitev $x(t)$ ne oscilira. $x(t)$ je torej oscilirajoča natanko tedaj, ko
    je $\lambda$ nerealna ničla. Po trditvi 5.3 je $\lambda$ nerealna ničla natanko tedaj, ko je 
    $\beta = \alpha\tau > 1/e$.
\end{proof}
