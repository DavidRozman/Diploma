...
podan kot funkcija na ustreznem intervalu. Pogoji za obstoj in enoličnost rešitve takega začetnega problema so podobni kot
...
condition is given as a function over a certain interval. The conditions for 
...
velikokrat pa se omejimo tudi zgolj na obstoj rešitve.
...
diferencialne enačbe z zamikom in si na preprostem zgledu pogledali primer reševanja in dobljeno rešitev primerjali z 
rešitvijo analogne navadno diferencialne enačbe. V podrobnejši analizi bomo navedli eksistenčni izrek, torej kdaj 
rešitev obstaja ter kakšne lastnosti ima iskana funkcija. Za tem si bomo pogledali nekaj osnovnih tipov enačb z 
njihovimi postopki za reševanje. Predstavili pa bomo tudi dva zgleda uporabe takih enačb pri modeliranju.


\section{Primerjava NDE in DDE}

Za začetek se spomnimo osnov navadnih diferencialnih enačb. Poglavje je povzeto po virih \cite{angdiploma} in 
\cite{wikide}.
...
\noindent Red enačbe je torej red najvišjega odvoda, ki nastopa v enačbi. Pridevnik \textit{navadna} se nanaša na dejstvo,
...
enačbi dodali začetni pogoj, zaradi katerega smo dobili enolično rešitev. Ta pogoj je bil podan kot vrednost funkcije
$x$ v točki. Standardna verzija eksistenčnega izreka pove, da je rešitev tovrstnega začetnega problema
...
se število parametrov v končnodimenzionalni splošni rešitvi zmanjša za ena.
...
V nadaljevanju diplomske naloge, bo izraz začetni problem pomenil diferencialno enačbo z
zamikom skupaj z začetnim pogojem.
...
\begin{zgled}
    ...
    Analogno nadaljujemo za $t \in [\tau, 2\tau]$. Tu velja $t-\tau \in [0,\tau]$, torej imamo
    \[\dot{x}(t)=-x(t-\tau)=-(1-(t-\tau))=(t-\tau)-1.\]
    Od tod sledi, da velja
...
\begin{proof}[Dokaz]
    ...
    postopka sledi, da je to edina možna rešitev.
\end{proof}
...
Poglavje je povzeto po virih \cite{angdiploma} in \cite{envir}.


Enačba je odvisna od dveh parametrov, zato jo bomo nekoliko poenostavili. Natančneje, z uvedbo nove spremenljivke
 $\mu:=\eta t, \eta > 0$, in funkcije
$U(\mu)=x(t)$ želimo enačbo preoblikovati tako, da bo njen zamik $\tau$ enak 1:
...
Sedaj na množici odvedljivih funkcij, definiramo sledeči linearni operator:
...
Enostavno je preveriti, da iz dejstva, da je $\lambda\in\mathbb{C}$ ničla te enačbe sledi, da je $e^{\lambda\mu}$ rešitev DDE $(1)$.
...
\begin{equation*}
    ...
    \end{split}      
\end{equation*}
Dobimo sistem enačb za realni in imaginarni del ničle:
\begin{equation*}
    \begin{split}
        a &= -\beta e^{-a}\cos(b) \\
        b &= \beta e^{-a}\sin(b).
...
\noindent Funkcija $h$ je holomorfna, zato zanjo veljajo naslednje lastnosti:
\begin{enumerate}
    ...
    Tedaj jih lahko uredimo v zaporedje, ki ga označimo z $\left\{\lambda_n\right\}$. Za to zaporedje velja
...
\begin{trditev}
    ...
    \begin{enumerate}
        \item Če je $\beta < 0$, potem obstaja natanko ena realna ničla karakteristične enačbe, ki je pozitivna.
        \item Če je $0 < \beta < 1/e$, potem obstajata natanko dve različni negativni realni
         ničli karakteristične enačbe, $\lambda_{1} < \lambda_{2}$, za kateri velja:
         \[ \lim_{\beta \to 0}\lambda_{1}=-\infty \quad \text{in} \quad \lim_{\beta \to 0}\lambda_{2}=0. \]
        \item Če je $\beta = 1/e$, potem je edina realna ničla karakteristične enačbe $\lambda = -1$ dvakratna.
...
\begin{proof}[Dokaz]
    ...
    \begin{enumerate}
        ...
        $\beta > 0$, za eno velja $\lambda = -\beta e^{-\lambda} <0$. Drugi del trditve sledi iz dejstva, da velja:
        ...
        Res, ničla je $\lambda = -1$. Velja pa tudi $h'(-1)=0$ in $h''(-1)=1$, torej
        je ničla reda 2.
...
Za $\beta > 0$ ima karakteristična enačba nekatere lepe lastnosti, kar nam pove naslednja trditev.
...
Rešitve enačbe iščemo v prvem kvadrantu, torej $a(\theta)>0$, $b(\theta) = \pi - \theta > 0$,
...
Ker je $\beta$ strogo padajoča na $(0,\pi/2)$, sledi da je njena zaloga vrednosti enaka $(\pi/2, \infty)$.
\end{proof}
...
\begin{trditev}
    ...
    \begin{enumerate}
        ...
        \item Če je $0<\alpha \tau <\pi/2$, je $x=0$ asimptotsko stabilna rešitev.
        \item Če je $\alpha \tau = \pi/2$, sta rešitvi $x=\sin(\pi(t/\tau)/2)$ in $x=\cos(\pi(t/\tau)/2)$.
        \item Če je $\alpha\tau > \pi/2$, je $x=0$ nestabilna rešitev.
    \end{enumerate}
\end{trditev}

\noindent Dokaz sledi direktno iz prejšnjih trditev in rezultata, ki pove, da je $x=0$ asimptotsko stabilna, če 
...
$|f(t)|\leq e^{at}$ za $a\in\R$ in $t\geq0$. Ker za $s\in\C$ velja $|e^{s}|=e^{\text{Re}(s)}$, imamo oceno
...
kjer je $|h(u,r)|\leq k$, $r\in(u-d,u+d).$
...
\begin{zgled}
    ...
    \begin{equation*}
        ...
        \end{split}      
    \end{equation*}
    Na levi strani enačbe uporabimo integracijo po delih. Pri tem upoštevamo, da za 
    rešitev velja $x(t)e^{-st} \to 0$, ko gre $t \to\infty$. Dobimo:
    \[\int_{0}^{\infty}\dot{x}(t)e^{-st}dt=-x(0)+s\int_{0}^{\infty}x(t)e^{-st}dt.\]
    Sedaj upoštevajmo še, da za $t\in[-1,0]$ velja $x(t)=\varphi(t)$ in enačbo (3) pretvorimo v naslednjo obliko:
    ...
    Za razliko od 3. poglavja, rešitev tokrat ni izražena 
    z elementarnimi funkcijami.
\end{zgled}
...
\begin{definicija}
    Sistem linearnih DDE prvega reda s konstantnimi koeficienti je podan z 
    \begin{equation} \label{eq5}
        \begin{split}
            \dot{x}(t)=Ax(t)+Bx(t-\tau)+f(t)\quad \text{za}\quad t\geq0,
        \end{split}
    \end{equation} kjer so
    $A,B\in\R^{n\times n}$, $\tau>0$ in $f$ zvezna funkcija $n$ spremenljivk. Začetni pogoj je podan s predpisom
    \begin{equation} \label{eq6}
        \begin{split}
            x(t)=\phi(t)\quad \text{za}\quad t\in [-\tau,0],
        \end{split}
    \end{equation}
    kjer je $\phi$ tokrat zvezna vektorska funkcija. Če je $f(t)\equiv0$, pravimo,
    da je sistem homogen.
\end{definicija}
Za analizo linearnih sistemov DDE lahko znova uporabimo Laplaceovo transformacijo. Najprej jo uporabimo na levi strani sistema:
\begin{equation*}
    \begin{split}
...
Nadalje lahko formuliramo in dokažemo tudi eksistenčni izrek, ki je zelo podoben tistemu iz tretjega poglavja, čeprav 
je linearni sistem DDE, ki ga obravnavamo,  bolj splošen od začetne enačbe prvega reda. Res, v njem 
imamo namreč tudi člen $x(t)$, kar pomeni, da je naš sistem odvisen tako od sedanjega kot tudi od preteklih časov. 
Seveda pa je v primeru, ko je $B=0$ to kar običajen sistem NDE s konstantnimi koeficienti.

\begin{izrek}
    Za $\tau>0$ je podan linearen sistem DDE s konstantnimi koeficienti
...
Oglejmo si zgled, ki ilustrira konstrukcijo rešitve iz zgornjega izreka.
\begin{zgled}
    ...
    =\begin{bmatrix}
        ...
        e^t & 2e^{2t} 
    \end{bmatrix}.\]
    Partikularno rešitev iščemo v obliki $x_p(t)=W(t)D(t)$, kjer je $D(t)$ neznana vektorska funkcija. Ko to vstavimo v enačbo, dobimo 
    \begin{equation*}
        \begin{split}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    Pripomnimo, da smo v zadnjem koraku brez škode za splošnost zanemarili vektor konstant, ki ga dobimo pri integraciji. 
    Partikularna rešitev je torej enaka
    \begin{equation*}
        \begin{split}
            x_p(t) &= WD \\
                    &= \begin{bmatrix}
        ...
        \end{split}
    \end{equation*}
    Za $t\in[\tau,2\tau]$ je splošna rešitev sistema torej enaka
    ...
    \end{bmatrix}.\]
    Z induktivnim postopkom bi dobili rešitev na poljubnem intervalu, ki je oblike $[n\tau, (n+1)\tau]$ za $n\in\N.$

\end{zgled}


Nazadnje izpeljimo še nekoliko splošnejši postopek za reševanje homogenih linearnih sistemov DDE oblike 
\[\dot{x}(t)=L(x_t),\]
...
$\lambda$ lastna vrednost matrike $L_\lambda$, oziroma, ko velja
\[ \text{det}(L_\lambda-\lambda I)=0,\]
vektor $v$ pa je njen lastni vektor.
Poglejmo si znova homogen sistem 
\[ \dot{x}(t)=Ax(t)+Bx(t-\tau),\]
z $A$ in $B$ $n\times n$ matriki in zamik $\tau>0$. Linearno preslikavo $L$ definiramo na naslednji način:
\[L(y)=Ay(0)+By(-\tau).\]
...
vektor matrike $A+e^{-\lambda\tau}B$. Opazimo, da je v primerjavi z reševanjem sistemov NDE s konstantnimi koeficienti ta postopek bolj zahteven,
saj karakteristična funkcija ni več polinom.
...
Populacijo nekega ekosistema želimo opisati s funkcijo časa $x(t)$. Spomnimo, eksponentni model predpostavlja, da je rast 
populacije premo sorazmerna z njeno velikostjo, podaja pa ga enačba
\[\dot{x}(t)=ax(t), \]
kjer je $a$ konstanta. V tem primeru za rešitev dobimo eksponentno funkcijo, ki pa le redko ustreza dejanskemu 
stanju v modelih iz narave.
Kadar je ekosistem omejen z viri, rast slej ko prej začne upočasnjevati, ko se populacija 
približa maksimalni nosilnosti okolja za neko dano vrsto. 
To maksimalno kapaciteto populacije označimo z $K>0$.
Iščemo torej model oblike $\dot{x}=f(x)$, pri katerem bo $f(x)\approx ax$ 
za majhne vrednosti funkcije $x$, v bližini rešitve $x=K$ pa se bo rast upočasnila. Najpreprostejša funkcija
s tema lastnostima je $f(x)=a-bx$, 
...
Poiščimo rešitve zgornje NDE.. Trivialni rešitvi sta $x(t)=0$ in $x(t)=K$. Da poiščemo še ostale, enačbo preoblikujemo:
...
Opazimo, da je rešitev ustrezna tudi za $x_0=0$ in $x_0=K$. Nadalje iz
...
rešitev logistične enačbe, $x=0$ pa nestabilna rešitev.

Navadna logistična enačba predpostavlja, da sprememba v velikosti populacije takoj vpliva na njeno rast, 
vendar pa se v naravi pogosto zgodi, da se tak vpliv zgodi z zamikom. Če to upoštevamo, dobimo naslednjo DDE:
...
$$\dot{x}(t)=ax(t)(1-bx_0),$$
torej je splošna rešitev te NDE enaka
$$x(t)=Ce^{a\left(1-x_0/K\right)t}.$$
Ob upoštevanju pogoja $x(0)=x_0$ dobimo 
...
Temu pritrjuje tudi spodnja slika, ki je bila izdelana v okolju Matlab.
...
\begin{figure}[H]
    \includegraphics[width=12cm, height=9cm]{slikca6.eps}
\end{figure}


\noindent Vidimo, da se v vseh primerih, razen v zadnjem, vrednost funkcije $x(t)$ na oscilirajoč način 
bliža nosilnosti $K=1$. Za $\alpha=\tau=1.5$ to ni res, saj je
$\alpha\tau=2.25>\pi/2$. Po Trditvi 4.5. lahko od tod sklepamo, da je $x=K$ nestabilna rešitev, saj 
se tako vede tudi njena linearna aproksimacija v bližini ravnovesne točke.
