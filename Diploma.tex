...
$|f(t)|\leq e^{at}$ za $t\geq0$. Ker za $s\in\C$ velja $|e^{s}|=e^{\text{Re}(s)}$, imamo
...
\end{lema}
Dokaz leme lahko bralec najde v viru \cite{dokazleme}
...
\subsection{Reševanje DDE z Laplaceovo transformacijo}
    ...
    \begin{equation} \label{eq3}
        \begin{split}
            \dot{x}(t)=x(t-1), \quad x(t)=\varphi(t), \quad t\in[0,1].
        \end{split}
    \end{equation}
    ...
    Sedaj upoštevajmo še, da za $t\in[0,1]$ velja $x(t)=\varphi(t)$. Enačbo (3) smo pretvorili na naslednjo obliko:
    \[(s-e^{-s})\int_{1}^{\infty}x(t)e^{-st}dt=\varphi(1)e^{-s}+e^{-s}\int_0^1\varphi(t)e^{-st}dt.\]
    Za $s-e^{-s}\neq 0$ dobimo torej:
    \[\int_{1}^{\infty}x(t)e^{-st}dt=\frac{\varphi(1)e^{-s}+e^{-s}\int_0^1\varphi(t)e^{-st}dt}{s-e^{-s}}.\]
