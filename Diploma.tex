...
\begin{izrek}
    ...
    Naj bo $\phi$ zvezno odvedljiva na $[0,\tau_m]$ in $f$ zvezna na $[0,\infty)$. 
    Označimo še $S = \{\sum_{i=1}^mj_i\tau_i|j_i\in\Z\}$ in  $S_1=S\cap(\tau_m,\infty)$.
    Potem obstaja natanko ena rešitev
    sistema za $t\geq0$, ki je zvezna in odvedljiva na $[0,\infty)$ razen morda na $S_1$.
\end{izrek}
...
\begin{proof}
    ...
    Ker je $f$ zvezna na $[0,\infty)$ in $\phi$ zvezno odvedljiva na $[0,\tau_m]$, je $g$ zvezna na 
    $[\tau_m,\tau_m+\tau_1]$. Po integraciji sledi, da obstaja enolična funkcija $x(t)$, ki zadošča sistemu (7) na 
    $(\tau_m,\tau_m+\tau_1)$ in začetnemu pogoju. Ta funkcija je zvezno odvedljiva na $[0,\tau_m+\tau_1]$, razen 
    morda v točki $t=\tau_m$. Sledi, da je 
    $g$ zvezna na $[\tau_m,\tau_m+2\tau_1]$ razen morda v točkah $t=\tau_m+\tau_j$, $j=1,\dots,m$, $t<\tau_m+2\tau_1$.
    Z integracijo 
    razširimo rešitev na $[0,\tau_m+2\tau_1]$. Tu je rešitev zvezna, zadošča sistemu na $(\tau_m,\tau_m+2\tau_1)$
    razen morda v točkah $t=\tau_m+\tau_j$, $j=1,\dots,m$, $t<\tau_m+2\tau_1$ in je odvedljiva na $[0,\tau_m+2\tau_1]$, razen 
    morda v točkah $t=\tau_m+\tau_j$, $j=0,\dots,m$, $t<\tau_m+2\tau_1$.
    Postopek lahko induktivno ponavljamo v neskočnčnost. Vidimo, da je
    rešitev $x(t)$ zvezna na $[0,\infty)$ in zvezno odvedljiva na $(0,\infty)$, razen morda na $S_1$.
...
\begin{zgled}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    Partikularno rešitev tokrat iščemo z nastavkom
    \[x_p(t)=\begin{bmatrix}
        p_1e^t+p_2e^{2t}+p_3  \\
        p_4e^{t}+p_5e^{2t}+p_6  
    \end{bmatrix}. \]
