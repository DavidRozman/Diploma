...
\begin{definicija}
    Sistem linearnih DDE je podan z $$\dot{x}=Ax(t)+Bx(t-\tau)+f(t)\quad \text{za}\quad t\geq0,$$ kjer je
    $A,B\in\R^{n\times n}$, $\tau>0$ in $f$ zvezna funkcija. Začetni pogoj je podan kot
    $$x(t)=\phi(t)\quad \text{za}\quad t\in [-\tau,0],$$ kjer je $\phi$ zvezna funkcija. Če je $f(t)=0$, pravimo,
